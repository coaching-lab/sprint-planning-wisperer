...
export const SprintTracker: React.FC = () => {
  const [sprints, setSprints] = useState<Sprint[]>([{
    ...
    velocity: 28,
    teamCapacity: 25,
    // (28 avg velocity × 90% availability) ÷ 100
    teamAvailability: 90,
    ...
    velocity: 30,
    teamCapacity: 28,
    // (28 avg velocity × 100% availability) ÷ 100
    ...
    velocity: 26,
    teamCapacity: 24,
    // (28 avg velocity × 85% availability) ÷ 100
    teamAvailability: 85,
  ...
  const handleAddSprint = (sprintData: Omit<Sprint, 'id' | 'completionRatio' | 'velocity'>) => {
    const completionRatio = sprintData.plannedPoints > 0 ? Math.round(sprintData.completedPoints / sprintData.plannedPoints * 100) : 0;
  ...
  return <div className="min-h-screen bg-background p-4 md:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2">Sprint Planning made easy</h1>
          <p className="text-muted-foreground">Track sprint performance, analyze velocity trends, and forecast future capacity to make your planning a breeze</p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={recentSprintsCount.toString()} onValueChange={value => setRecentSprintsCount(Number(value))}>
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {[...sprints].sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime()).slice(0, Math.min(recentSprintsCount, sprints.length)).map(sprint => <div key={sprint.id} className="flex items-center justify-between">
                        <span className="font-medium">{sprint.name}</span>
                        <div className="flex items-center gap-2">
                          <div className="w-24 bg-muted rounded-full h-2">
                            <div className="h-2 rounded-full bg-gradient-to-r from-accent to-success transition-all duration-500" style={{
                          width: `${sprint.completionRatio}%`
                        }} />
                          </div>
          ...
          <TabsContent value="sprints">
            <SprintList sprints={sprints} recentSprintsCount={recentSprintsCount} onEdit={sprint => {
            setEditingSprint(sprint);
            setShowForm(true);
          }} onDelete={handleDeleteSprint} onManageSprints={handleManageSprints} />
          </TabsContent>

          <TabsContent value="trends">
            <VelocityChart sprints={sprints} detailed recentSprintsCount={recentSprintsCount} />
          </TabsContent>
        ...
        {/* Sprint Form Modal */}
        {showForm && <SprintForm sprint={editingSprint} sprints={sprints} onSubmit={editingSprint ? handleEditSprint : handleAddSprint} onCancel={() => {
        setShowForm(false);
        setEditingSprint(undefined);
      }} />}

        {/* Manage Sprints Modal */}
        {showManageSprints && <ManageSprints sprints={sprints} onSave={handleSaveManageSprints} onClose={() => setShowManageSprints(false)} />}
      </div>
    </div>;
};